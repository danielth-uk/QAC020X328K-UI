...
const AdminClients = () => {
  ...
  useEffect(() => {
    axios.get("/api/admin/users/client").then((response) => {
      setUsers(response.data);
    });
    axios.get("/api/admin/orgs").then((response) => {
      var temp = [];
      response.data.forEach((row) => {
        if (row["org"] !== "qa") temp.push(row["org"]);
      });
      setOrgs(temp);
      setSelectedOrg(temp[0]);
    });
    setLoading(false);
  }, [forceReload]);
    ...
    <>
      ...
      <div className="md:container mx-auto mt-14">
        ...
        <div className="container" style={{ display: "flex" }}>
          ...
          >
            {orgs.map((element) => (
              <option value={element} key={"option-" + element}>
        ...
        <div className="mt-4">
          <table className="table w-full bg-base-200">
            ...
            {loading ? (
              <TableLoading columns={3} key={"loadingTable"} />
            ) : (
              <>
                {users.length === 0 ? (
